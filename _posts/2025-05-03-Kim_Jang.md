...
<style>
.scroll-stage { width: 100%; height: 100vh; overflow: hidden; background-color: black; position: relative; }
.scroll-container { display: flex; flex-direction: row; width: 100%; height: 100vh; }
.scroll-frame { width: 65%; height: 100vh; position: relative; overflow: hidden; }
.map-slide { position: absolute; top: 0; left: 0; width: 100%; height: 100%; object-fit: cover; opacity: 0; transition: opacity 1s ease-in-out; }
.map-slide.active { opacity: 1; }
.timeline-box { width: 35%; padding: 3rem 2rem; display: flex; flex-direction: column; justify-content: flex-start; overflow-y: auto; color: white; }
.timeline-step { opacity: 0; transform: translateY(50px); transition: all 0.8s ease-out; margin-bottom: 4rem; }
.timeline-step.visible { opacity: 1; transform: translateY(0); }
.timeline-step video, .timeline-step img { width: 100%; margin-top: 1rem; border-radius: 8px; }
.scroll-followup { max-width: 700px; margin: 5rem auto; padding: 2rem; font-size: 1.1rem; line-height: 1.6; opacity: 0; transform: translateY(60px); transition: all 1s ease-in-out; }
.scroll-followup.visible { opacity: 1; transform: translateY(0); }
</style>
